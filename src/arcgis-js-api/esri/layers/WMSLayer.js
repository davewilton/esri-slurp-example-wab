//>>built
define(["require", "dojo/_base/kernel", "dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/has", "../kernel", "../request", "../urlUtils", "../SpatialReference", "../geometry/Extent", "./DynamicMapServiceLayer", "./WMSLayerInfo", "dojo/query"], function(s, h, t, p, f, x, y, u, m, n, k, v, w) {
    return t([v], {
        ...
        getImageUrl: function(a, b, c, d) {
            if (!this.visibleLayers || 0 === this.visibleLayers.length) d(this._blankImageURL);
                ...
                if (f.some(this._WEB_MERCATOR, function(a) {
                    ...
                    var g = f.filter(this.spatialReferences, function(a) {
                        return f.some(this._WEB_MERCATOR, function(b) {
                            return b == a
                        })
                    }, this);
                    0 === g.length && (g = f.filter(this.spatialReferences, function(a) {
                        return f.some(this._WORLD_MERCATOR, function(b) {
                            return b == a
                        })
                    }, this));
                    e = 0 < g.length ? g[0] : this._WEB_MERCATOR[0]
                }
                this.spatialReferences = f.filter(this.spatialReferences,
                    function(a) {
                        return a !== e
                    });
                ...
                b = e ? e : NaN;
                isNaN(b) || ("1.3.0" == this.version ? a.CRS = "EPSG:" + b : a.SRS = "EPSG:" + b);
                "1.3.0" ==
                this.version && this._useLatLong(b) ? a.BBOX = q + "," + g + "," + r + "," + l : a.BBOX = g + "," + q + "," + l + "," + r;
                b = this.getMapURL;
                var h;
                b += -1 == b.indexOf("?") ? "?" : "";
        ...
        _initLayer: function(a, b) {
            this.spatialReference = new n(this.extent.spatialReference);
        ...
        _getCapabilities: function() {
            ...
            var b = this._url.path + "?",
                c;
            for (c in a) a.hasOwnProperty(c) && (b += "?" == b.substring(b.length - 1, b.length) ? "" : "\x26", b += c + "\x3d" + a[c]);
            u({
                url: b,
                handleAs: "xml",
                headers: {
            ...
            if (a) {
                ...
                if (b = this._getTag("Layer", a)) {
                    ...
                    if (c) {
                        ...
                        if (!this.allExtents || !this.allExtents.length) c.allExtents = [], f.forEach(this.layerInfos[0].allExtents, function(a,
                            b) {
                        ...
                        if (!this.spatialReferences.length && 0 < this.layerInfos.length)
                            for (b = 0; b < this.layerInfos.length; b++) {
                                var g = this.layerInfos[b];
                                this.spatialReferences = this.layerInfos[0].spatialReferences;
                                if (this.spatialReferences.length) break;
                                if (g.subLayers && 0 < g.subLayers.length) {
                                    var l;
                                    for (l = 0; l < g.subLayers.length && !(this.spatialReferences = g.subLayers[l].spatialReferences, this.spatialReferences.length); l++);
                                }
                                if (this.spatialReferences.length) break
                            }
                    }
                    this.getMapURL = this._getCapabilitiesURL;
                    if ((b = h.query("DCPType", this._getTag("GetMap", a))) && 0 < b.length)
                        if ((b = h.query("HTTP", b[0])) && 0 < b.length)
                            if ((b = h.query("Get", b[0])) && 0 < b.length)
                                if (b = this._getAttributeValue("OnlineResource", "xlink:href", b[0], null)) b.indexOf("\x26") == b.length - 1 && (b = b.substring(0, b.length - 1)), this.getMapURL = b;
                    this.getMapFormats = [];
                    0 === h.query("Operation", a).length ? f.forEach(h.query("Format", this._getTag("GetMap", a)), function(a) {
                        this.getMapFormats.push(a.text ?
                            a.text : a.textContent)
                    }, this) : f.forEach(h.query("Operation", a), function(a) {
                        "GetMap" == a.getAttribute("name") && f.forEach(h.query("Format", a), function(a) {
                    ...
                    this._initLayer()
                } else this._errorHandler(Error("esri.layers.WMSLayer: Response does not contain any layers."))
            } else this._errorHandler(Error("GetCapabilities request for " +
                this._getCapabilitiesURL + " failed. (Response is null.)"))
        },
        _getCapabilitiesError: function(a) {
            a && a.message && (a.message = "GetCapabilities request for " + this._getCapabilitiesURL + " failed. (" + a.message + ")");
        ...
        _getLayerInfo: function(a) {
            ...
            var d = this._getTag("EX_GeographicBoundingBox",
                    a),
                e;
            d && (e = new k(0, 0, 0, 0, new n({
                wkid: 4326
            })), e.xmin = parseFloat(this._getTagValue("westBoundLongitude", d, 0)), e.ymin = parseFloat(this._getTagValue("southBoundLatitude", d, 0)), e.xmax = parseFloat(this._getTagValue("eastBoundLongitude", d, 0)), e.ymax = parseFloat(this._getTagValue("northBoundLatitude", d, 0)), b.allExtents[0] = e);
            !c && !d && (e = new k(-180, -90, 180, 90, new n({
                wkid: 4326
            })), b.allExtents[0] = e);
            b.extent = b.allExtents[0];
            var g = -1 < f.indexOf(["1.0.0", "1.1.0", "1.1.1"], this.version) ? "SRS" : "CRS";
            f.forEach(a.childNodes,
                function(a) {
                    ...
                    } else if (a.nodeName == g) a = (a.text ? a.text : a.textContent).split(" "), f.forEach(a, function(a) {
        ...
        _getExtent: function(a, b, c) {
            ...
            if (a) {
                ...
                var e = parseFloat(a.getAttribute("minx")),
                    g = parseFloat(a.getAttribute("miny")),
                    f = parseFloat(a.getAttribute("maxx"));
                a = parseFloat(a.getAttribute("maxy"));
                c ? (d.xmin = isNaN(g) ? -1 * Number.MAX_VALUE : g, d.ymin = isNaN(e) ? -1 * Number.MAX_VALUE : e, d.xmax = isNaN(a) ? Number.MAX_VALUE : a, d.ymax = isNaN(f) ?
                    Number.MAX_VALUE : f) : (d.xmin = isNaN(e) ? -1 * Number.MAX_VALUE : e, d.ymin = isNaN(g) ? -1 * Number.MAX_VALUE : g, d.xmax = isNaN(f) ? Number.MAX_VALUE : f, d.ymax = isNaN(a) ? Number.MAX_VALUE : a);
                d.spatialReference = new n({
                    wkid: b
                })
            }
            return d
        ...
        _getTagValue: function(a, b, c) {
            return (a = h.query(a,
                b)) && 0 < a.length ? a[0].text ? a[0].text : a[0].textContent : c
        },
        _getAttributeValue: function(a, b, c, d) {
            return (a = h.query(a, c)) && 0 < a.length ? a[0].getAttribute(b) : d
        },
        ...
        _stripParameters: function(a, b) {
            var c = m.urlToObject(a),
                d, e = [];
            for (d in c.query) c.query.hasOwnProperty(d) && -1 === f.indexOf(b,
                d.toLowerCase()) && e.push(d + "\x3d" + c.query[d]);
